...
object TcpServer1 extends zio.App {
...
object TcpClient1 extends zio.App {
  ...
  def program = 
    for {
      ...
      message = Chunk.fromArray("message from client " .getBytes)
      _ <- connect(publicAddress).use(connection => 
          currentDateTime
            .map(date =>
              "message from client " + date
            )
            .flatMap(msg => 
              connection.send(Chunk.fromArray(msg.getBytes))
